    ...
    if (data.cards.length > 0) {
      data.cards.forEach(ele => {
        if (
          ele.card_type === 9 &&
          ele.mblog !== undefined &&
          ele.mblog.page_info !== undefined &&
          ele.mblog.page_info.type === "video" &&
          /\[第(\d)+次\]/.test(ele.mblog.page_info.media_info.next_title)
        ) {
          let title = ele.mblog.page_info.media_info.next_title.match(
            /第(\d)+次/
          )[0];
          let url =
...
const downloadVideo = function(options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    let stream = fs.createWriteStream(
      path.join(__dirname, `../dist/video/${options.title}.mp4`)
    );
    request(options.url)
      .pipe(stream)
      .on("error", () => {
        console.log(
          `视频${chalk.yellow(options.title)}${chalk.red("下载失败")}...`
        );
        reject();
      })
      .on("close", () => {
        console.log(
          `视频${chalk.yellow(options.title)}${chalk.green("下载完成")}...`
        );
        resolve();
      });
  });
